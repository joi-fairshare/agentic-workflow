...
allowed-tools: Bash(source ~/.claude/skills/*), Bash(source ~/.codex/skills/*), Bash(source ~/.Codex/skills/*), Read, Write, Glob, AskUserQuestion
---
...
BRIDGE_OK=false
if command -v claude &>/dev/null && claude mcp list 2>/dev/null | grep -q "agentic-bridge"; then
  BRIDGE_OK=true
elif command -v codex &>/dev/null && codex mcp list 2>/dev/null | grep -q "agentic-bridge"; then
  BRIDGE_OK=true
elif lsof -i TCP:3100 -sTCP:LISTEN &>/dev/null; then
  BRIDGE_OK=true
fi
...
```bash
SKILL_PATH="${CODEX_HOME:-$HOME/.codex}/skills/design-mockup-ios/SKILL.md"
[ -f "$HOME/.claude/skills/design-mockup-ios/SKILL.md" ] && SKILL_PATH="$HOME/.claude/skills/design-mockup-ios/SKILL.md"
[ -f "$HOME/.Codex/skills/design-mockup-ios/SKILL.md" ] && SKILL_PATH="$HOME/.Codex/skills/design-mockup-ios/SKILL.md"
SHARED_DIR="$(dirname "$(readlink -f "$SKILL_PATH")")/../_shared"
LOCK_NAME=ios-sim
