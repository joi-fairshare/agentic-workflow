...
- Read `AGENTS.md` if it exists; otherwise read `CLAUDE.md` if it exists
...
Work through each question sequentially. For each one, present your analysis based on the project context, then pause and wait for the user's response before moving on. This is a conversation -- do not try to force it into a tool-style multiple-choice exchange; present each question naturally and wait.
...
Based on response:
- Architectural concerns → open and follow [archReview](../archReview/SKILL.md)
- Product/founder lens → open and follow [productReview](../productReview/SKILL.md)
- Both → follow `archReview` and then `productReview` in sequence
- Neither → done
