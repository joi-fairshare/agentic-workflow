---
name: addressReview
description: Address PR review comments by spawning domain-specific implementation agents in parallel. Reads from .review-cache/<pr>.json as source of truth, merges any new human GitHub comments, implements fixes, and updates the state file. Can be re-run to continue the review loop.
...
After all implementers complete, update `.review-cache/{number}.json`:
