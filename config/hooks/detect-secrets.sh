...
if echo "$COMMAND" | grep -qE 'A(KIA|SIA)[0-9A-Z]{16}'; then
...
if echo "$COMMAND" | grep -qE '(gh[posur]_[A-Za-z0-9_]{36,}|github_pat_[A-Za-z0-9_]{22,})'; then
...
if echo "$COMMAND" | grep -iqE "(export\s+)?(API_KEY|SECRET|TOKEN|PASSWORD|PRIVATE_KEY|ACCESS_KEY|SECRET_KEY)\s*=\s*['\"]?[A-Za-z0-9+/=_.,-]{20,}"; then
