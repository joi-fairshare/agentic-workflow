...
- **`scripts/serena-docker`** — Wrapper script that launches the Serena Docker container, mounting the target repo read-only at invocation time. The base image is `serena-local:latest`; an optional C# extension image (`serena-local:csharp`) is built separately via `BUILD_CSHARP=1 ./setup.sh`.
...
- **Two-image strategy** — The base image supports TypeScript, Python, Go, Rust, and most other languages. The C# extension image adds OmniSharp and is opt-in to keep base image size manageable.
